/*	Задание:
	...
	if (oSession.HostnameIs("habrahabr.ru") && oSession.oResponse.headers.ExistsAndContains("Content-Type", "text/html")) {
...
(function () {
	...
	function jQueryAndDOMReady() {
		if (location.href === "http://habrahabr.ru/") {
			if ( !( (window.$ !== undefined) && document.documentElement ) ) {
				setTimeout(function () {
					jQueryAndDOMReady();
				}, 0);
			} else {
				...
				$( function () {
					// Данная проверка нужна чтобы избежать рекурсивного выполнения ф-ции внутри ифреймов
					if (location.href === "http://habrahabr.ru/") {
